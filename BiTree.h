...
//根据先序序列和中序序列生成二叉树
BiTNode *premid(char pre[], char mid[], int pre_i, int mid_i, int n, int *preIndex) {
    if (n <= 0)
        return NULL;
    ...
    root->lchild = premid(pre, mid, pre_i + 1, mid_i, i, preIndex);
    root->rchild = premid(pre, mid, pre_i + i + 1, mid_i + i + 1, n - i - 1, preIndex);
